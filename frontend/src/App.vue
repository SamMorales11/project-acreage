...
// Pustaka baru untuk PDF yang mendukung CSS Modern (oklch)
...
const form = ref({ luas_tanah: 100, luas_bangunan: 80, kamar_tidur: 3, kamar_mandi: 2, lokasi_skor: 2 })
...
let map = null;
let marker = null;
const centerPoint = [-1.2379, 116.8529]; 
...
const updateLocationScore = (lat, lng) => {
  ...
  if (distance <= 3) { form.value.lokasi_skor = 3; currentTierName.value = 'Tier 3 • Central Business District'; } 
  else if (distance <= 8) { form.value.lokasi_skor = 2; currentTierName.value = 'Tier 2 • Urban Residential'; } 
  else { form.value.lokasi_skor = 1; currentTierName.value = 'Tier 1 • Outskirts / Rural'; }
};

const initMap = () => {
  const bounds = [[-1.4, 116.6], [-1.1, 117.1]];
  map = L.map('map', { zoomControl: true, maxBounds: bounds, minZoom: 11 }).setView(centerPoint, 13); 
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { attribution: '&copy; CartoDB' }).addTo(map);
  L.circle(centerPoint, { color: '#3b82f6', fillColor: '#3b82f6', fillOpacity: 0.15, radius: 3000, weight: 2, dashArray: '5, 5' }).addTo(map);
  L.circle(centerPoint, { color: '#6366f1', fillColor: '#6366f1', fillOpacity: 0.05, radius: 8000, weight: 1, dashArray: '5, 5' }).addTo(map);
  const startPos = [-1.2450, 116.8600];
  marker = L.marker(startPos, { draggable: true }).addTo(map);
  updateLocationScore(startPos[0], startPos[1]);
  marker.on('dragend', () => { const position = marker.getLatLng(); updateLocationScore(position.lat, position.lng); });
};

const formatRupiah = (number) => {
  return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(number * 1000000);
}
...
const getPrediction = async () => {
  ...
  try {
    const payload = { luas_tanah: Number(form.value.luas_tanah), luas_bangunan: Number(form.value.luas_bangunan), kamar_tidur: Number(form.value.kamar_tidur), kamar_mandi: Number(form.value.kamar_mandi), lokasi_skor: Number(form.value.lokasi_skor) }
...
const loadAnalytics = async () => {
  try {
    ...
    if (response.data.labels && response.data.labels.length > 0) renderChart(response.data.labels, response.data.values)
...
// --- FUNGSI GENERATE PDF (Menggunakan html-to-image) ---
const generatePDF = async () => {
  ...
  try {
    ...
    if (!reportElement) {
      throw new Error("Target area dokumen tidak ditemukan di layar.");
    }
    
    // Menggunakan html-to-image yang support oklch dan CSS modern
    ...
    // Kalkulasi rasio tinggi gambar berdasarkan elemen asli
    const elemWidth = reportElement.offsetWidth;
    const elemHeight = reportElement.offsetHeight;
    ...
    pdf.text('ACREAGE. VALUATION REPORT', 10, 20);
    
    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(148, 163, 184); 
    pdf.text(`Date Generated: ${new Date().toLocaleString('id-ID')}`, 10, 28);
    
    // Spesifikasi
    pdf.text(`Subject Property: ${form.value.luas_tanah}m2 Lot | ${form.value.luas_bangunan}m2 Building`, 10, 36);
    pdf.text(`Architecture: ${form.value.kamar_tidur} Beds | ${form.value.kamar_mandi} Baths | Location: ${currentTierName.value}`, 10, 42);
    
    // Masukkan Gambar
    pdf.addImage(imgData, 'PNG', 10, 50, pdfWidth - 20, pdfHeight);
    
    pdf.save(`Acreage_Valuation_${Date.now()}.pdf`);
    
  } catch (error) {
    console.error("Gagal membuat PDF:", error);
    alert("Gagal mengunduh laporan. Pesan sistem: " + error.message);
  } finally {
    isGeneratingPDF.value = false;
  }
}

const debouncedPredict = debounce(() => { getPrediction(); }, 300);
watch(form, () => { debouncedPredict(); }, { deep: true });
  ...
  chartInstance = new Chart(ctx, {
    ...
    data: {
      labels: labels.map(l => {
        ...
        const proLabels = { luas_tanah: 'Lot Area', luas_bangunan: 'Building Area', kamar_tidur: 'Bedrooms', kamar_mandi: 'Bathrooms', lokasi: 'Location Tier', lokasi_skor: 'Location Tier' };
        return proLabels[normalizedKey] || String(l).toUpperCase();
      }),
      datasets: [{ label: ' Feature Impact (%)', data: values, backgroundColor: '#3b82f6', hoverBackgroundColor: '#60a5fa', borderRadius: 50, barPercentage: 0.4, borderSkipped: false }]
    },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: { backgroundColor: 'rgba(15, 23, 42, 0.95)', titleFont: { size: 12, family: 'Inter', color: '#94a3b8' }, bodyFont: { size: 14, weight: 'bold', family: 'Inter' }, padding: 12, cornerRadius: 8, displayColors: false } },
      animation: { duration: 400, easing: 'easeOutQuart' },
      scales: { x: { grid: { display: false, drawBorder: false }, ticks: { display: false } }, y: { grid: { display: false, drawBorder: false }, ticks: { color: '#94a3b8', font: { size: 11, weight: '600', family: 'Inter', tracking: 'widest' }, padding: 10 } } }
    }
  })
}

onMounted(() => { initMap(); loadAnalytics(); loadMetrics(); getPrediction(); })
</script>
    ...
    <nav class="sticky top-0 z-50 bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-800 px-6 py-4">
      <div class="max-w-7xl mx-auto flex justify-between items-center">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-xl font-bold tracking-tight text-white uppercase">Acreage<span class="text-blue-500">.</span></h1>
        </div>
    ...
    <main class="flex-grow w-full max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <button @click="generatePDF" :disabled="isGeneratingPDF" 
          class="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-5 py-2.5 rounded-xl border border-slate-600 transition-all hover:shadow-lg disabled:opacity-50 text-xs font-bold uppercase tracking-widest group">
          <svg v-if="isGeneratingPDF" class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <form @submit.prevent class="space-y-8 relative z-10">
            <div class="space-y-3">
              ...
              <div class="relative bg-slate-800/50 rounded-xl p-3 border border-slate-700/50 mb-3 shadow-inner">
                <input v-model="form.luas_tanah" type="number" class="w-full text-center text-6xl font-black text-blue-400 font-mono focus:outline-none bg-transparent" min="30" max="1000">
            ...
            <div class="space-y-3">
              ...
              <div class="relative bg-slate-800/50 rounded-xl p-3 border border-slate-700/50 mb-3 shadow-inner">
                <input v-model="form.luas_bangunan" type="number" class="w-full text-center text-6xl font-black text-blue-400 font-mono focus:outline-none bg-transparent" min="20" max="800">
                <span class="absolute right-4 bottom-2 text-sm font-semibold text-slate-500 font-mono">m²</span>
              </div>
            ...
            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-3">
                <label class="flex justify-between items-center text-[11px] font-bold text-slate-400 uppercase tracking-wider"><span>Beds</span><span class="text-blue-400 font-mono text-lg font-bold">{{ form.kamar_tidur }}</span></label>
              ...
              <div class="space-y-3">
                <label class="flex justify-between items-center text-[11px] font-bold text-slate-400 uppercase tracking-wider"><span>Baths</span><span class="text-blue-400 font-mono text-lg font-bold">{{ form.kamar_mandi }}</span></label>
        ...
        <section class="lg:col-span-8 space-y-6" id="report-content">
          ...
          <div class="bg-[#1e293b] rounded-2xl border border-slate-700/50 p-6 shadow-xl">
            <div class="flex justify-between items-end mb-6">
              <div>
                <h3 class="text-sm font-bold text-white uppercase tracking-wider mb-1">Model Performance</h3>
                <p class="text-[11px] text-slate-400">Academic validation metrics</p>
              </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> R-Squared</h4>
                <p class="text-2xl font-black text-emerald-400 font-mono mt-2">{{ (modelMetrics.r2 * 100).toFixed(1) }}<span class="text-sm text-emerald-600">%</span></p>
              </div>
              <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                 <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-amber-400"></span> MAE</h4>
                <p class="text-lg font-black text-amber-400 font-mono mt-2 truncate"><span class="text-xs text-amber-600 mr-1">±</span>{{ formatRupiah(modelMetrics.mae).replace('Rp', '').trim() }}</p>
              </div>
              <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                 <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-rose-400"></span> RMSE</h4>
                <p class="text-lg font-black text-rose-400 font-mono mt-2 truncate"><span class="text-xs text-rose-600 mr-1">±</span>{{ formatRupiah(modelMetrics.rmse).replace('Rp', '').trim() }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-[#1e293b] rounded-2xl border border-slate-700/50 p-6 shadow-xl">
            <div class="flex justify-between items-end mb-6">
              <div>
                ...
                <p class="text-[11px] text-slate-400">Random Forest feature importance distribution</p>
              </div>
            </div>
            <div class="h-[260px] w-full"><canvas id="importanceChart"></canvas></div>
          </div>
    ...
    <footer class="border-t border-slate-800 bg-[#0f172a] py-8 mt-auto">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-2">
          ...
          <span class="px-2 py-0.5 rounded-md bg-slate-800 text-[10px] font-mono text-slate-400 border border-slate-700">v5.2.0</span>
        </div>
        <div class="text-center md:text-right">
          ...
          <p class="text-[10px] text-slate-600 mt-1 font-mono">
            STACK: VUE / FASTAPI / SCIKIT-LEARN / JSPDF
          </p>
        </div>
      </div>
    </footer>
...
<style scoped>
.custom-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 16px; height: 16px; border-radius: 50%; background: #3b82f6; cursor: pointer; border: 2px solid #0f172a; box-shadow: 0 0 10px rgba(59, 130, 246, 0.5); transition: transform 0.1s ease-in-out; }
.custom-slider::-moz-range-thumb { width: 16px; height: 16px; border-radius: 50%; background: #3b82f6; cursor: pointer; border: 2px solid #0f172a; box-shadow: 0 0 10px rgba(59, 130, 246, 0.5); transition: transform 0.1s ease-in-out; }
...
:deep(.leaflet-container) { background-color: #0f172a; font-family: 'Inter', sans-serif; }
:deep(.leaflet-control-attribution) { background-color: rgba(15, 23, 42, 0.7) !important; color: #64748b !important; border-radius: 4px 0 0 0; }
:deep(.leaflet-control-attribution a) { color: #3b82f6 !important; text-decoration: none; }
</style>